...
resource "aws_cloudfront_distribution" "website_cdn_root" {
  ...
  custom_error_response {
    ...
    error_code            = 404
    response_page_path    = var.prafuls-web ? "/index.html" : "/error.html"
    response_code         = var.prafuls-web ? 200 : 404
  }
...
resource "aws_s3_bucket" "website_root" {
  ...
  website {
    ...
    error_document = var.prafuls-web ? "" : "error.html"
  }
  tags = merge(var.tags, {
    ManagedBy = "terraform"
    Changed   = formatdate("YYYY-MM-DD hh:mm ZZZ", timestamp())
...
resource "aws_s3_bucket" "website_redirect" {
  ...
  bucket        = var.website-domain-redirect
  acl           = "public-read"
  force_destroy = true
...
resource "aws_s3_bucket_object" "file" {
  for_each     = fileset(var.web_file, "**")
  bucket       = data.aws_s3_bucket.website_root.bucket
  key          = each.value
